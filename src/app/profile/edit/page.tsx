...
import type { UpdateProfileRequestBody } from "@/types/api.types";
...
export default function FlexibleProfileForm({
  ...
  // Choose schema based on editing mode - both allow partial completion
  ...
  const transformFormData = useCallback(
    (data: UpdateProfileFormData): UpdateProfileRequestBody => {
      return {
        name: user?.name,
        avatar: user?.avatar,
        profile: {
          ...
          idDetails: data.idDetails?.idNumber ? data.idDetails : undefined,
        },
      };
    },
    [user]
  ...
  const getStepForField = (field: string): FormStep | null => {
    const stepFieldMap: Record<FormStep, string[]> = {
      [FormStep.BASIC_INFO]: ["role", "bio"],
      [FormStep.LOCATION]: ["location"],
      [FormStep.CONTACT]: ["contactDetails", "socialMediaHandles"],
      [FormStep.IDENTIFICATION]: ["idDetails"],
      [FormStep.REVIEW]: [],
    };
    ...
    async (data: UpdateProfileFormData) => {
      ...
      try {
        ...
        toast({
          ...
          description: "You can always update your information later.",
        });
  ...
  const currentCompleteness = React.useMemo(() => {
    ...
    return calculateProfileCompleteness(formData);
  }, [formData]);
    ...
    (step: FormStep): number => {
      ...
      switch (step) {
        ...
        case FormStep.LOCATION:
          return formData.location?.ghanaPostGPS ? 100 : 0;
        case FormStep.CONTACT:
          return formData.contactDetails?.primaryContact ? 100 : 0;
        case FormStep.IDENTIFICATION:
          return formData.idDetails?.idNumber ? 100 : 0;
        default:
          return 0;
      }
    },
              ...
              <div className="flex justify-between items-center pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="flex space-x-3">
                  ...
                  {/* <button
                    ...
                    className="px-6 py-2 text-sm font-medium text-white bg-green-600 dark:bg-green-500 border border-transparent rounded-md hover:bg-green-700 dark:hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    {isSubmitting ? (
                      <>
                      ...
                      <>{getSubmissionCTA(currentCompleteness)}</>
                    )}
                  </button> */}
                </div>
              </div>
